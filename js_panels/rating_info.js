...
function on_mouse_lbtn_up(x, y) {
	...
	if (TextBtn_info.isXYInButton(x, y)) {
		if (fb.IsPlaying) {
			fb.RunMainMenuCommand("激活正在播放项目") || fb.RunMainMenuCommand("Activate now playing");
...
function on_mouse_lbtn_dblclk(x, y) {
	if (g_metadb && TextBtn_info.isXYInButton(x, y)) fb.RunContextCommandWithMetadb("属性", g_metadb) || fb.RunContextCommandWithMetadb("Properties", g_metadb);
}
...
function on_mouse_rbtn_up(x, y) {
	if (TextBtn_info.isXYInButton(x, y)) {
		...
		if(fso.FileExists(fb.ProfilePath +"assemblies\\Mp3tag\\Mp3tag.exe") && (tracktype < 2) && (follow_cursor || !fb.IsPlaying))
		...
		case 1:
			...
			break;
		case 2:
			if (g_metadb) fb.RunContextCommandWithMetadb("属性", g_metadb) || fb.RunContextCommandWithMetadb("Properties", g_metadb);
			break;
		case 3:
			is_mood = !is_mood;
