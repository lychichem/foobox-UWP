...
function AboutFoobox() {
	...
	var HelpFile = fb.ProfilePath + "foobox帮助.chm";
...
function on_mouse_lbtn_up(x, y) {
	...
	if (buttons[4].MouseUp()) fb.RunMainMenuCommand("打开...") || fb.RunMainMenuCommand("Open...");
	//buttons[5].MouseUp();
	...
	if (ui_noborder) {
		if (buttons[0].MouseUp()) {
			...
			fb_hWnd.SendMsg(0x0010,0,0);//fb.Exit();
		}
...
function init_buttons() {
	...
	buttons[3].OnClick = function(x, y) {
		...
		if(fso.FileExists(fb.ProfilePath + "assemblies\\Mp3tag\\Mp3tag.exe")){
		...
		case (ret == 1319):
			...
			WshShell.Run("\"" + fb.ProfilePath + "assemblies\\Mp3tag\\Mp3tag.exe" + "\"", false);
			break;
		}
		buttons[3].Reset();
		basemenu.Dispose();
