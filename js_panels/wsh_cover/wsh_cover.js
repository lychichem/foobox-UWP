...
/*
		...
		var txt_font = gdi.Font("Tahoma",Math.floor(w/7),0);
		...
		g.DrawString("5  E D I T I O N",gdi.Font("Tahoma",Math.floor(w/26),0), RGBA(0,0,0,255), margin_left+txt_title_w/2,margin_top+txt_title_h+10,txt_title_w,txt_title_h)
		*/
		var cover_img_default = gdi.Image(fb.ProfilePath + "themes\\foobox\\images\\cover_default.jpg")
	...
	if (this.Properties.MenuButton.Show) {
		this.menuButton = new function(x, y, w, h, prop) {
			...
			var opacity = 255;
			var opacity2 = 0;
			var fmt = StrFmt(1, 2, 100, 0); //prop.MacTypeSupport ? 1|4|100 : StrFmt(1, 2, 100, 0);
			var imgCache = {
				img: null,
				text: ""
			};
			...
			this.Draw = function(g) {
				...
				if (!imgCache.img || imgCache.text != this.caption) {
					...
					g2.DrawString(this.caption, prop.Font, prop.CaptionColor, 0, -4, imgCache.img.width, imgCache.img.height, fmt); // This is foreground text.
			...
			this.OnClick = function(x, y) {
				menuShowing = true;
				this.menu.Pop(this.x - 27, this.y + this.h);
				menuShowing = false;
			}
		...
		}(0, 0, 82, 26, this.Properties.MenuButton);
...
function Controller(imgArray, imgDisplay, prop) {
	...
	if (!this.Properties.SingleImageMode) {
		this.imgSwitchMenu = new function() {
			...
			this.Build = function() {
				...
				} else if (imgArray.Properties.GenrePathFormat) baseMenu.AppendMenuItem(MF_DISABLED, id++, "流派" + " (0)");
	...
	this.funcMenu = new function() {
		...
		this.Build = function() {
			...
			} else baseMenu.AppendMenuItem(MF_DISABLED, id++, "管理内嵌图像");
...
/*
            ...
            subMenu_FC.AppendTo(baseMenu, MF_POPUP, "光标跟随模式");
            
            funcs[id+1] = [_this.SetFollowCursorProperties, 1];
            subMenu_FC.AppendMenuItem(MF_STRING, id+1, "仅在非播放时");
            funcs[id+2] = [_this.SetFollowCursorProperties, 2];
            subMenu_FC.AppendMenuItem(MF_STRING, id+2, "总是");
            funcs[id] = [_this.SetFollowCursorProperties, 0];
            subMenu_FC.AppendMenuItem(MF_STRING, id, "从不");
	...
	this.SearchFromWeb = function(arr) {
		...
		if (si.GetUrl) url = si.GetUrl(arr[1]);
		else {
	...
	function OnNewTrack(metadb) {
		if (metadb && currentMetadb && currentMetadb.Compare(metadb)) _this.SwitchCover(0);
		else {
			isNewgroup = false;
			currentIndex = -1;
...
var themePath = fb.ProfilePath + "themes\\foobox\\js_panels";
//------------------------------------------------
var VBE;
try {
...
var Properties = new function() {
		...
		//if (this.Panel.WorkDirectory) this.PanelWorkDirectory = fb.ProfilePath + this.PanelWorkDirectory;
		...
		this.Controller = {
			...
			FollowCursor: fbx_set[10] ? 2 : 1,
			//window.GetProperty("Panel.FollowCursor", 1),
			Cycle: {
		...
		if (typeof(this.Controller.FollowCursor) != "number") this.Controller.FollowCursor = 1;
		else if (this.Controller.FollowCursor < 0) this.Controller.FollowCursor = 0;
		else if (this.Controller.FollowCursor > 2) this.Controller.FollowCursor = 2;
		...
		this.GetFont = function() {
			...
			//font.Dispose();
			return gdi.Font(fbx_set[13], 14, 1);
		}
		...
		//var defaultGenrePathFormat = fb.ProfilePath + 'MusicArt\\Other\\*.*';//'$directory_path(%path%)\\cover.jpg';
		this.Image = {
		...
		//this.Image.BuildinPathFormat = this.Image.BuildinPathFormat.replace(/%foobar_path%/ig, fb.ProfilePath.slice(0,-1));
		this.Image.GenrePathFormat = this.Image.GenrePathFormat.replace(/%foobar_path%/ig, fb.ProfilePath.slice(0, -1)); // Process %foobar_path% field.
...
if (searchScripts) eval(searchScripts);
...
function on_paint(gr) {
	...
	gr.drawline(0, 0, 0, wh, 1, RGBA(0, 0, 0, 100));
...
function on_playlist_switch() {
	if (!fb.IsPlaying || MainController.Properties.FollowCursor == 2) {
		metadb = fb.GetFocusItem();
		MainController.OnSelectionChanged(metadb);
	}
}
