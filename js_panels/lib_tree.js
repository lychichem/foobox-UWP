		...
		if (cust_icon_font.length) {
			...
			try {
				var st = Math.round(parseFloat(cust_icon_font[1])),
					font_test = GdiFont(cust_icon_font[0], 16, st);
...
function panel_operations() {
	...
	var view_ppt = [
		...
		window.GetProperty(" View 09: Name // Pattern", "按日期 // $date($replace(%date%,/,-,.,-))|[%album artist% - ]%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%"), 
...
function populate() {
	...
	this.load = function(list, type, add, send, def_pl, insert) {
		...
		if (fb.IsPlaying && !add && fb.GetNowPlaying()) {
			...
			plman.EnsurePlaylistItemVisible(pln, !insert || plman.GetPlaylistFocusItemIndex(pln) == -1 ? plman.PlaylistItemCount(pln) - items.Count : plman.GetPlaylistFocusItemIndex(pln) - items.Count);
		}
		...
		this.on_char = function(code) {
			...
			if (!p.s_search) {
				...
				timer.jsearch = window.SetTimeout(function() {
					for (i = 0; i < l; i++) {
						if (pop.tree[i].name != "所有音乐" && pop.tree[i].name.substring(0, jSearch.length).toLowerCase() == jSearch.toLowerCase()) {
							found = true;
							pos = i;
							pop.tree[i].sel = true;
							pop.get_sel_items();
...
function menu_object() {
	...
	this.OptionsTypeMenu = function(Menu, StartIndex) {
		...
		for (i = 0; i < p.menu.length; i++) {
			...
			if (i < p.menu.length - 1) Menu.AppendMenuItem(MF_STRING, Index, p.menu[i]);
			Index++;
	...
	this.rbtn_up = function(x, y) {
		...
		SettingMenu.AppendTo(menu, MF_STRING | MF_POPUP, "常用设定"); {
			...
			CountMenu.AppendTo(SettingMenu, MF_STRING | MF_POPUP, "数目显示设置");{
			...
			SettingMenu.AppendMenuSeparator();
			DClickMenu.AppendTo(SettingMenu, MF_STRING | MF_POPUP, "双击操作设置");{
		...
		if (show_context) {
			...
			OptionsMenu.AppendTo(menu, MF_STRING | MF_POPUP, "选项");
			menu.AppendMenuSeparator();
			var items = fb.CreateHandleList();
			try {
		...
		if (idx > 5800) {
			...
			case 5802:
				p.base = !p.base;
				window.SetProperty(" Node: Show All Music", p.base);
				lib.get_library();
				lib.rootNodes();
				break;
			case 5803:
